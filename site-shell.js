...
function buildShell(){
 ...
 const applySettings=settings=>{
  ...
  if(values.subtitle)header.querySelector('.scav-site-subtitle').textContent=values.subtitle;
   ...
   const allowedPreviewOrigins=['http://127.0.0.1:4173','http://localhost:4173','https://scavlandfanbase.github.io'];
   window.addEventListener('message',event=>{if(!allowedPreviewOrigins.includes(event.origin)||event.data?.type!=='scavland-live-settings-preview'||!event.data.settings)return;applySettings({global:event.data.settings})});
   ...
   fetch('https://demtoqsafufzmnhvaykj.supabase.co/rest/v1/site_visual_assets?asset_key=eq.header-banner&select=public_url',{headers:{apikey:'sb_publishable_0kdLCpTy7Sf8BKkIU5TOqw_Qaay4gzH'}}).then(r=>r.ok?r.json():[]).then(rows=>{const url=rows?.[0]?.public_url;if(url)document.documentElement.style.setProperty('--site-header-image',`url('${url.replace(/'/g,'%27')}')`)}).catch(()=>{});
  fetch('data/site-content.json',{cache:'no-store'}).then(r=>r.ok?r.json():null).then(content=>{const pageContent=content?.pages?.[page];if(!pageContent)return;if(page==='index.html'&&Array.isArray(pageContent.sections)){const footer=document.querySelector('footer');pageContent.sections.forEach(section=>{const el=document.querySelector('[data-section-id="'+CSS.escape(section.id)+'"]');if(el&&footer)footer.before(el)})}const title=document.querySelector('.hero h1,main h1,body>h1'),intro=document.querySelector('.hero p,main>section>p,main>p');if(pageContent.title&&title)title.textContent=pageContent.title;if(pageContent.intro&&intro)intro.textContent=pageContent.intro;const grid=document.querySelector('.grid'),cards=[...document.querySelectorAll('.grid .card')],sourceCards=pageContent.sections?.find(section=>section.id==='feature-grid')?.cards||[];if(page==='index.html'&&grid){while(cards.length<sourceCards.length){const a=document.createElement('a');a.className='card';a.href='#';const h=document.createElement('h3'),p=document.createElement('p');a.append(h,p);grid.append(a);cards.push(a)}while(cards.length>sourceCards.length){cards.pop()?.remove()}}sourceCards.forEach((card,index)=>{const target=cards[index];if(!target)return;target.dataset.contentId=card.id||'';if(card.title)target.querySelector('h3')?.replaceChildren(document.createTextNode(card.title));if(card.description)target.querySelector('p')?.replaceChildren(document.createTextNode(card.description));if(card.href){if(target.tagName==='A')target.href=card.href;target.onclick=()=>location.href=card.href}});if(page==='roadmap.html')pageContent.sections?.forEach((section,index)=>{const column=document.querySelectorAll('.roadmap-column')[index];if(!column)return;if(section.title)column.querySelector('h2')?.replaceChildren(document.createTextNode(section.title));(section.cards||[]).forEach((card,cardIndex)=>{const item=column.querySelectorAll('li')[cardIndex];if(!item)return;item.querySelector('strong')?.replaceChildren(document.createTextNode(card.title||''));item.querySelector('span')?.replaceChildren(document.createTextNode(card.description||''))})});const update=pageContent.sections?.find(section=>section.id==='latest-update');if(update?.title)document.querySelector('.updates h2')?.replaceChildren(document.createTextNode(update.title));
  ...
  if(page==='factions.html')pageContent.sections?.flatMap(section=>section.cards||[]).forEach(card=>{const factionId=card.id.replace(/-description$/,''),box=document.getElementById(factionId),tile=document.querySelector('.faction-tile.faction-'+factionId);if(box&&card.description){box.querySelector('.faction-title')?.replaceChildren(document.createTextNode(card.title||''));box.querySelector('.faction-text')?.replaceChildren(document.createTextNode(card.description||''))}if(tile&&card.image){tile.querySelector('h2')?.replaceChildren(document.createTextNode(card.title||''));const image=tile.querySelector('img');if(image)image.src=card.image}});
  if(page==='vendors.html'){const rates=pageContent.sections?.find(section=>section.id==='selling-guide')?.cards||[],safe=value=>String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));rates.forEach(card=>{const target=document.querySelector('.sell-card[data-vendor-id="'+safe(card.vendorId)+'"]');if(!target)return;target.innerHTML='<strong>'+safe(card.title)+'</strong>'+String(card.description||'').split('\n').map(line=>'<div>'+safe(line).replace(/(\d+%)/g,'<span class="sell-rate">$1</span>')+'</div>').join('')})}
  ...
  pageContent.sections?.forEach(section=>{if(!section.target)return;const target=document.querySelector(section.target);if(!target)return;if(section.image&&target.tagName==='IMG')target.src=section.image;if(section.title&&target.tagName!=='IMG')target.textContent=section.title;const sibling=target.nextElementSibling;if(section.intro&&sibling&&sibling.tagName==='P')sibling.textContent=section.intro});
  }).catch(()=>{});
