...
function validPage(value: unknown) { if (!value || typeof value !== "object" || Array.isArray(value)) return false; const page = value as any; if (!text(page.title, 200) || !text(page.intro, 200)) return false; if (!Array.isArray(page.sections)) return false; return page.sections.every((section: any) => section && typeof section.id === "string" && text(section.title, 200) && (!section.cards || Array.isArray(section.cards) && section.cards.every((card: any) => card && typeof card.id === "string" && text(card.title, 160) && text(card.description, 500) && imagePath(card.image) && (!card.href || typeof card.href === "string")))); }
Deno.serve(async request => {
  ...
  if (!allowedPages.has(body?.page) || !validPage(body?.content)) return respond({ error: "Invalid public content update." }, 400);
  ...
  document.pages = { ...(document.pages || {}), [body.page]: body.content };
