...
function validPage(value: unknown) { if (!value || typeof value !== "object" || Array.isArray(value)) return false; const page = value as any; if (!text(page.title, 200) || !text(page.intro, 200)) return false; if (!Array.isArray(page.sections)) return false; return page.sections.every((section: any) => section && typeof section.id === "string" && text(section.title, 200) && text(section.intro, 200) && imagePath(section.image) && (!section.summary || typeof section.summary === "string" && section.summary.length <= 300) && (!section.url || typeof section.url === "string" && section.url.length <= 500) && (!section.cards || Array.isArray(section.cards) && section.cards.every((card: any) => card && typeof card.id === "string" && text(card.title, 160) && text(card.description, 500) && imagePath(card.image) && (!card.href || typeof card.href === "string")))); }
function validGlobal(value: unknown) { if (!value || typeof value !== "object" || Array.isArray(value)) return false; const global = value as any; return text(global.footerText, 200) && Array.isArray(global.navigation) && global.navigation.every((item: any) => item && allowedPages.has(item.page) && typeof item.label === "string" && item.label.length <= 40); }
