...
(() => {
  ...
  const config={
    items:{label:'item',file:'items',fields:[['estimatedPrice','Estimated price','integer'],['rank','Rank','integer'],['maxStack','Maximum stack','integer'],['stackable','Can stack','boolean'],['effect-health','Health effect','signed'],['effect-bleed','Bleed effect','signed'],['effect-radiation','Radiation effect','signed'],['effect-hunger','Hunger effect','signed'],['effect-thirst','Thirst effect','signed'],['description','Description','textarea'],['notes','Notes','textarea']]},
  ...
  function open(record){
    ...
    html+=config.fields.map(([k,l,t])=>field(k,l,t,k.startsWith('effect-')?selected.effects?.[k.slice(7)]:selected[k]??(k==='inventoryDocumented'?false:null))).join('');
  ...
  function picture(id,value,label,portrait=false){
    ...
    const listId='picture-options-'+id;
      holder.className='picture';holder.innerHTML='<label>Find a picture by filename<input class="picture-search" type="search" placeholder="Start typing a filename..." value="'+esc(value?value.split('/').pop():'')+'" autocomplete="off"><div class="picture-results" role="listbox" aria-label="Matching pictures"></div></label><label>'+esc(label)+'<select aria-label="'+esc(label)+'"><option value="">No picture</option>'+list.map(p=>'<option value="'+esc(p)+'"'+(p===value?' selected':'')+'>'+esc(p.split('/').pop().replaceAll('__',' · ').replaceAll('-',' '))+'</option>').join('')+'</select></label><label>Or paste a GitHub picture path<input class="picture-path" type="text" placeholder="images/items/example.png" value="'+esc(value)+'" spellcheck="false"></label><span class="help">Type a filename, then click a matching result. You can also paste a path or upload a picture.</span><span class="path-status help" role="status"></span><label>Or upload from your device<input type="file" accept="image/png,image/jpeg,image/webp"></label><span class="help">PNG, JPG or WebP, up to 5 MB. Uploaded when you save.</span><img alt="Selected picture"'+(!value?' hidden':'')+'>';
    const select=holder.querySelector('select'),searchInput=holder.querySelector('.picture-search'),results=holder.querySelector('.picture-results'),img=holder.querySelector('img'),pathInput=holder.querySelector('.picture-path'),pathStatus=holder.querySelector('.path-status');state.select=select;
    function show(){pathInput.value=state.value||'';searchInput.value=state.value?state.value.split('/').pop():'';pathInput.setCustomValidity('');pathStatus.textContent='';img.hidden=!state.value;if(state.value)img.src=uploads.get(state.value)?.url||state.value;else img.removeAttribute('src');}
    ...
    function renderPictureOptions(query=''){const needle=query.trim().toLowerCase(),matches=list.filter(path=>!needle||path.split('/').pop().toLowerCase().includes(needle)||path.toLowerCase().includes(needle));select.innerHTML='<option value="">No picture</option>'+matches.map(path=>'<option value="'+esc(path)+'"'+(path===state.value?' selected':'')+'>'+esc(path.split('/').pop().replaceAll('__',' · ').replaceAll('-',' '))+'</option>').join('');results.innerHTML=needle?(matches.length?matches.map(path=>'<button type="button" class="picture-result" data-path="'+esc(path)+'"><b>'+esc(path.split('/').pop())+'</b><span>'+esc(path)+'</span></button>').join(''):'<span class="help">No matching pictures.</span>'):'';results.querySelectorAll('.picture-result').forEach(button=>button.onclick=()=>{state.value=button.dataset.path;select.value=state.value;show();renderPictureOptions(state.value.split('/').pop());sizePortrait();changed();});return matches;}
      searchInput.oninput=()=>{const query=searchInput.value,match=renderPictureOptions(query).find(path=>path.split('/').pop().toLowerCase()===query.trim().toLowerCase());if(match){state.value=match;select.value=match;show();renderPictureOptions(state.value.split('/').pop());sizePortrait();changed();}else pathStatus.textContent=query.trim()?'Choose a matching picture from the results below.':'';};
      renderPictureOptions(value?value.split('/').pop():'');
    show();
  ...
  function collect(){
    ...
    const effects={};
    for(const [k,,type]of config.fields){const v=$('f-'+k).value.trim();const value=type==='boolean'?(v===''?null:v==='true'):['integer','number','signed'].includes(type)?(v===''?null:Number(v)):(v||(['location','vendorRank'].includes(k)?'':null));if(k.startsWith('effect-')){if(value!==null)effects[k.slice(7)]=value;}else next[k]=value;}
    if(kind==='items')next.effects=Object.keys(effects).length?effects:null;
