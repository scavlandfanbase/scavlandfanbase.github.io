...
(() => {
  ...
  function picture(id,value,label,portrait=false){
    ...
    holder.className='picture';holder.innerHTML='<label>Find a picture by filename<input class="picture-search" type="search" list="'+listId+'" placeholder="Start typing a filename..." value="'+esc(value?value.split('/').pop():'')+'" autocomplete="off"><datalist id="'+listId+'">'+list.map(p=>'<option value="'+esc(p.split('/').pop())+'" label="'+esc(p)+'"></option>').join('')+'</datalist></label><label>'+esc(label)+'<select aria-label="'+esc(label)+'"><option value="">No picture</option>'+list.map(p=>'<option value="'+esc(p)+'"'+(p===value?' selected':'')+'>'+esc(p.split('/').pop().replaceAll('__',' · ').replaceAll('-',' '))+'</option>').join('')+'</select></label><label>Or paste a GitHub picture path<input class="picture-path" type="text" placeholder="images/items/example.png" value="'+esc(value)+'" spellcheck="false"></label><span class="help">Use the filename search, paste a path, or upload a picture.</span><span class="path-status help" role="status"></span><label>Or upload from your device<input type="file" accept="image/png,image/jpeg,image/webp"></label><span class="help">PNG, JPG or WebP, up to 5 MB. Uploaded when you save.</span><img alt="Selected picture"'+(!value?' hidden':'')+'>';
    const select=holder.querySelector('select'),searchInput=holder.querySelector('.picture-search'),img=holder.querySelector('img'),pathInput=holder.querySelector('.picture-path'),pathStatus=holder.querySelector('.path-status');state.select=select;
