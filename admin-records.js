...
(() => {
  ...
  function open(record){
    ...
    if(kind==='vendors')html+='<fieldset class="record-wide"><legend>Stock list</legend><p class="help">Choose items by name. Leave an unknown price blank. Adding a portrait or shop picture does not verify these stock rows.</p><div id="stock"></div><button id="add-stock" type="button">Add stock item</button></fieldset><details class="record-wide"><summary>Shop screenshots</summary><p class="help">Existing review details are preserved. New screenshots are added as pending review.</p><div id="shots"></div><button id="add-shot" type="button">Add shop screenshot</button></details>';
  ...
  function picture(id,value,label,portrait=false){
    ...
    holder.className='picture';holder.innerHTML='<label>'+esc(label)+'<select aria-label="'+esc(label)+'"><option value="">No picture</option>'+list.map(p=>'<option value="'+esc(p)+'"'+(p===value?' selected':'')+'>'+esc(p.split('/').pop().replaceAll('__',' · ').replaceAll('-',' '))+'</option>').join('')+'</select></label><label>Or upload from your device<input type="file" accept="image/png,image/jpeg,image/webp"></label><span class="help">PNG, JPG or WebP, up to 5 MB. Uploaded when you save.</span><img alt="Selected picture"'+(!value?' hidden':'')+'>';
    const select=holder.querySelector('select'),img=holder.querySelector('img');state.select=select;
    function show(){img.hidden=!state.value;img.src=state.value?uploads.get(state.value)?.url||state.value:'';}
    show();
    ...
    holder.querySelector('input').onchange=async e=>{const file=e.target.files[0];if(!file)return;if(!['image/png','image/jpeg','image/webp'].includes(file.type)||file.size>5*1024*1024){$('message').textContent='Choose a PNG, JPG or WebP picture up to 5 MB.';e.target.value='';return;}reading++;$('save').disabled=true;try{const url=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(file);});const path='evidence-inbox/admin/'+(slug($('f-name').value)||'picture')+'-'+crypto.randomUUID()+'.'+({'image/png':'png','image/jpeg':'jpg','image/webp':'webp'}[file.type]);uploads.set(path,{path,content:url.split(',')[1],url});for(const control of pictureControls.values())control.select?.add(new Option(file.name,path));select.value=path;state.value=path;show();await sizePortrait();changed();}catch{$('message').textContent='This picture could not be read. Please choose it again.';}finally{reading--;if(!reading&&!busy)$('save').disabled=false;}};
  }
  function itemSelect(id){return '<select class="item-select" aria-label="Item" required><option value="">Choose an item</option>'+items.slice().sort((a,b)=>a.name.localeCompare(b.name)).map(i=>'<option value="'+esc(i.id)+'"'+(i.id===id?' selected':'')+'>'+esc(i.name)+'</option>').join('')+'</select>';}
  ...
  function renderStock(){$('stock').innerHTML=stock.map(row=>'<div class="row stock-row">'+itemSelect(row.itemId)+'<label>Unlock rank<input class="rank" value="'+esc(row.rank)+'"></label><label>Price<input type="number" min="0" step="1" class="price" value="'+esc(row.price)+'"></label><label class="record-wide">Details<input class="details" value="'+esc(row.details)+'"></label><span class="help">'+(row.source?.status==='screenshot-verified'?'Existing screenshot evidence retained':'Needs verification')+'</span><button type="button">Remove</button></div>').join('');$('stock').querySelectorAll('button').forEach((b,i)=>b.onclick=()=>{readStock();stock.splice(i,1);renderStock();changed();});}
